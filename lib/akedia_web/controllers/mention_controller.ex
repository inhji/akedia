defmodule AkediaWeb.MentionController do
  ...
  def save_valid_mention(conn, params) do
    post =
      case Regex.run(~r/posts\/(.*)/, params["target"]) do
        [_, id] ->
          Posts.get_post!(id)
    ...
    if post == nil do
      conn
      |> send_resp(400, "This is not the host you're looking for (object could not be found)")
    else
  ...
  def is_valid_mention(conn, params) do
    if String.trim(to_string(params["target"])) == "" do
      {:error,
       conn |> send_resp(400, "This is not the host you're looking for (target is blank)")}
    else
      if String.trim(to_string(params["source"])) == "" do
        {:error,
         conn |> send_resp(400, "This is not the host you're looking for (source is blank)")}
      else
        ...
        if target_uri.host != my_uri.host do
          {:error,
           ...
           |> send_resp(
             400,
             "This is not the host you're looking for (host is not equal to my host)"
           )}
        else
          if Webmentions.is_valid_mention(params["source"], params["target"]) do
            ...
            {:error,
             conn
             |> send_resp(400, "This is not the host you're looking for (mention is not valid)")}
          end
        end
      end
    end
